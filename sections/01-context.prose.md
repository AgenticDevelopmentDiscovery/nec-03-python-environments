...
and `site-packages`.  
- `conda` extends this to non-Python and binary
dependencies.  
- `uv` is a fast modern resolver that builds environments and
lockfiles in seconds.  
- Whatever the tool, a **lockfile** is what makes it
...
Before `venv` [@pep405] — standard since Python 3.3 — every project on a machine shared
...
reach for `conda` instead.  
- The difference between pinning a dependency
...
rebuilds it from the lock to prove the point.  
- It does not cover PyPI
packaging or container-level reproducibility (Docker).  
